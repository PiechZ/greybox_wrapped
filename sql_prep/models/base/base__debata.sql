...
debates_in_years as (
    select
        *,
        date_sub(
            'year', DATE '{{ var("first_school_year") }}', datum
        ) as school_year
    from debates
),
...
ballots as (
    select
        ...
        sum(
            case when has_voted_affirmative then 1 else 0 end
        ) as affirmative_ballots,
        count(*)
        - sum(
            case when has_voted_affirmative then 1 else 0 end
        ) as negative_ballots
...
single_ballot_debates as (
    select
        ...
        case
            ...
            when
                judges.has_voted_affirmative and not judges.is_persuasive
                then 2
            when
                not judges.has_voted_affirmative and not judges.is_persuasive
                then 1
            when
                not judges.has_voted_affirmative and judges.is_persuasive
                then 0
...
multi_ballot_debates as (
    select
        ...
        ballots.total_ballots as judge_count,
        ballots.affirmative_ballots
        > ballots.negative_ballots as is_affirmative_win,
        ballots.affirmative_ballots = 0
        or ballots.negative_ballots = 0 as is_persuasive_win,
        ballots.affirmative_ballots = ballots.negative_ballots as is_draw,
        case
            when
                ballots.affirmative_ballots > ballots.negative_ballots
                and ballots.negative_ballots = 0
                then 3
            when
                ballots.affirmative_ballots > ballots.negative_ballots
                and ballots.negative_ballots > 0
            ...
            when
                ballots.affirmative_ballots < ballots.negative_ballots
                and ballots.affirmative_ballots > 0
                then 1
            when
                ballots.affirmative_ballots < ballots.negative_ballots
                and ballots.affirmative_ballots = 0
                then 0
            when ballots.affirmative_ballots = ballots.negative_ballots then 1.5
        end as affirmative_ballots_normalized
    from ballots
    where ballots.total_ballots > 1
),
...
final as (
    select
        ...
        3
        - all_debates.affirmative_ballots_normalized as negative_ballots_normalized,
    ...
    left join
        competitions
        on debates_in_years.soutez_id = competitions.soutez_id
